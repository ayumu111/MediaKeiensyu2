...
class ResourceManager:
    ...
    def _load_image(self, path, size=None, transparent=False):
        try:
            loaded = pygame.image.load(path)
            # 透過指定: アルファを尊重しつつ、スケール後の左上色をカラ―キーで抜く
            has_alpha = loaded.get_alpha() or loaded.get_masks()[3] != 0
            img = loaded.convert_alpha() if has_alpha else loaded.convert()
            if size:
...
class Phase4Scene(Scene):
    ...
    def _draw_ui(self):
        ...
        # カメラエラーメッセージ
        if not self.camera_ready:
            msg = self.renderer.render(
                "カメラが見つかりません。接続を確認してください。", 24, Config.RED
            )
            shadow = self.renderer.render(
                "カメラが見つかりません。接続を確認してください。", 24, Config.BLACK
            )
