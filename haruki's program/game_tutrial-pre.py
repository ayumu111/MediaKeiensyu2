...
class Config:
    ...
    SCREEN_HEIGHT = 600
    FPS = 60
    CAPTION = "Pose Battle Game - Layout Configurable"
...
class ResourceManager:
    ...
    def get_system_font(self, size):
        if size in self.fonts_system:
            return self.fonts_system[size]
        try:
            font = pygame.font.SysFont("meiryo", size)
        except:
            font = pygame.font.Font(None, size)
        self.fonts_system[size] = font
        return font
...
class TextRenderer:
    ...
    def render_system(self, text, size, color, bold=False):
        font = self.rm.get_system_font(size)
        font.set_bold(bold)
